...
def old_update_or_create(model_data):
    ...
    for data in model_data:
        obj, _created = ExampleBulkModel.objects.update_or_create(
            ...
            defaults={
                "something_non_unique": data["something_non_unique"]
            }
        )
        if _created:
            created.append(obj)
    ...
    def _fake_bulk_model_data(n_models):
        ...
        while len(data) < n_models:
            ...
            data_record = factory.build(
                dict,
                FACTORY_CLASS=factories.ExampleBulkModelFactory,
            )
            if next((x for x in data if x["something_unique"] == data_record["something_unique"]), None):
                continue
...
class TestBulkUpdateOrCreate():
    ...
    N_QUERIES = 3  # should never do more than 3 queries: 1 to check exisiting objects, 1 to update, 1 to create
    ...
    def test_update_objects(self, fake_bulk_model_data, django_assert_max_num_queries):
        ...
        assert ExampleBulkModel.objects.count() == 10
        assert ExampleBulkModel.objects.filter(something_non_unique="updated").count() == 10

    def test_update_and_create_objects(self, fake_bulk_model_data, django_assert_max_num_queries):
    ...
    def test_comparator_fn(self, fake_bulk_model_data):
        ...
        comparator_fn = lambda matching_object, data_record: data_record["something_non_unique"] == "passes"

        # should only update 1 instance b/c the comparator_fn will fail w/ test_data[1]
        created, updated = bulk_update_or_create(ExampleBulkModel, test_data, comparator_fn=comparator_fn)
        assert len(created) == 0
        assert len(updated) == 1

        assert ExampleBulkModel.objects.get(something_unique=test_data[0]["something_unique"]).something_non_unique != "passes"
    ...
    def test_faster(self, fake_bulk_model_data):
        ...
        t = time()
        bulk_create_created, bulk_create_updated = bulk_update_or_create(ExampleBulkModel, test_data)
        bulk_create_time = time() - t

        # new method of updating
        t = time()
        bulk_update_created, bulk_update_updated = bulk_update_or_create(ExampleBulkModel, test_data)
