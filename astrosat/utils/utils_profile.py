...
def profile(n_calls=100, sort_key="cumulative", path=""):
    ...
    valid_sort_keys = functools.reduce(
        operator.add,
        [key._all_values for key in pstats.SortKey]
    )
        ...
        def track_memory_wrapper(*args, **kwargs):
            ...
            try:
                ...
                for class_name in memory_info["stats"].tracked_classes:
                    ...
                    if history:
                        classes_stats.append({
                            ...
                            "size": pp(snapshot.classes.get(class_name, {}).get('sum', 0)),
                        })
                ...
                for class_stats in classes_stats:
                    print("{name}: created/deleted {n_instances} times for a min/max of {min_instances}/{max_instances} instances: {size:>10}".format(**class_stats), file=stream)
